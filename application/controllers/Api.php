...
class Api extends REST_Controller{
    ...
    public function user_post(){
        ...
        $detail_user = $this->m_user->where_id_user($id_user)->with_user()->get();
        $this->response($detail_user);
    }
    ...
    public function recomendation_post(){
        ...
        if ($jenis_layanan=='menu') {
            ...
            $data_recomendation = $this->m_kuliner->
         // where('harga <=',$budget_menu)->with_kuliner()->get_all();
            where('harga_atas <=',$budget_menu)->with_menu()->paginate(15);
    ...
    public function rekomendasi_post(){
        ...
        for ($j=0; $j < count($id_tujuan); $j++) { 
            ...
            if ($jenis_layanan=='wisata'){
                ...
                $data[$j] = $this->m_rekomendasi->insert(array('id_user'=> $id_user, 'id_tujuan' => $id_tujuan[$j], 'jenis_layanan' => $jenis_layanan));
            }
        ...
        switch ($jenis_layanan) {
            case 'wisata':
                //$this->response($this->m_rekomendasi->where(array('id_user' => $id_user,'id_'.$jenis_layanan, 'flag' => '0'))->with_wisata()->get_all());
                $this->response($this->m_rekomendasi->where('id_rekomendasi', $data)->with_wisata()->get_all());
                break;
            case 'kamar': 
                //$this->response($this->m_rekomendasi->where(array('id_user' => $id_user,'id_'.$jenis_layanan, 'flag' => '0'))->with_kamar(array('with'=>array('relation'=>'penginapan')))->get_all());
                $this->response($this->m_rekomendasi->where('id_rekomendasi', $data)->with_kamar(array('with'=>array('relation'=>'penginapan')))->get_all());
                break;
            case 'menu':
                //$this->response($this->m_rekomendasi->where(array('id_user' => $id_user,'id_'.$jenis_layanan, 'flag' => '0'))->with_menu(array('with'=>array('relation'=>'kuliner')))->get_all());
                //$this->response($this->m_rekomendasi->where('id_rekomendasi', $data)->with_menu(array('with'=>array('relation'=>'kuliner')))->get_all());
                $this->response($this->m_rekomendasi->where('id_rekomendasi', $data)->with_kuliner()->get_all());
                break;
            default:
                break;
        }
    ...
    public function get_rekomendasi_post(){
        $this->load->model('m_rekomendasi');
        ...
        for ($j=0; $j < count($id_tujuan); $j++) { 
            ...
            if ($jenis_layanan=='wisata'){
                $this->response($this->m_rekomendasi->where(array('id_user'=> $id_user, 'id_tujuan'=> $id_tujuan[$j], 'flag'=>'0'))->with_wisata()->get_all());
            }

            if ($jenis_layanan=='kamar') {
                //$this->m_rekomendasi->insert(array('id_user' => $id_user, 'id_tujuan' => $id_tujuan[$j], 'jenis_layanan' => $jenis_layanan));
                $this->response($this->m_rekomendasi->where(array('id_user'=> $id_user, 'id_tujuan'=> $id_tujuan[$j], 'flag'=>'0'))->with_kamar()->get_all());
            }

            if ($jenis_layanan=='menu') {
                //$this->m_rekomendasi->insert(array('id_user' => $id_user, 'id_tujuan' => $id_tujuan[$j], 'jenis_layanan' => $jenis_layanan));
                $this->response($this->m_rekomendasi->where(array('id_user'=> $id_user, 'id_tujuan'=> $id_tujuan[$j], 'flag'=>'0'))->with_kuliner()->get_all());
            }
        }
    }
    ...
    public function pesanan_post(){
        ...
        $posisi_lat     = $this->post('posisi_lat');
        $posisi_lng     = $this->post('posisi_lng');
        $total_budget   = $this->post('total_budget');
        $status         = $this->post('status');

        $data = $this->m_pesanan->insert(array('id_customer' => $id_customer, 'posisi_lat' => $posisi_lat, 'posisi_lng' => $posisi_lng, 'total_budget' => $total_budget));

        //$this->response($this->m_pesanan->where(array('id_customer' => $id_customer, 'status' => '0'))->with_user()->with_rekomendasi()->get());

        //$this->response($this->m_pesanan->where(array('id_customer' => $id_customer, 'status' => '0'))->with_user(array('with'=>array('relation'=>'rekomendasi')))->get());
        // $this->response($this->m_pesanan->where(array('id_customer' => $id_customer, 'status' => '0'))->with_rekomendasi(array('with'=>array('relation'=>'user')))->get());
        //$this->response($this->m_pesanan->where(array('id_customer' => $id_customer, 'status' => '0'))->get());
        $this->response($this->m_pesanan->where('id_pesanan', $data)->get());
    }



    public function terima_post(){
        $this->load->model('m_pesanan');
        ...
        $data = array(
            ...
            'waktu_acc' => $this->post('waktu_acc'),
            'total_budget' => $this->post('total_budget'),
            'status' => $this->post('status')
        );

        $this->m_pesanan->update($data, $id_pesanan);

        $this->response($this->m_pesanan->where('id_pesanan', $id_pesanan)->where->get());
    }

     public function get_pesanan_post(){
        $this->load->model('m_pesanan');

        $id_pesanan = $this->post('id_pesanan');
        $id_customer = $this->post('id_driver');
        $status_selesai = $this->post('status_selesai');
        ...
        $this->response($this->m_pesanan->where('id_pesanan', $id_pesanan)->where('status_selesai', $status_selesai)->get());
    }
