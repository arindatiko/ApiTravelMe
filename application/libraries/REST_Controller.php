...
abstract class REST_Controller extends CI_Controller {
     ...
     * return
     */
    protected $_response = array('status' => FALSE, 'message' => 'Oops, something went wrong.');
    protected $_not_found = 'Data tidak ditemukan.';
    ...
    public function validDate($date, $format = 'Y-m-d H:i:s')
	{
        if ($date) {
            ...
            if (!($d && $d->format($format) == $date)) {
                $this->form_validation->set_message(__FUNCTION__, 'Tanggal/Hari Pelayanan tidak valid.');
                return false;
            }
    ...
    public function validPoli($poli)
    {
        if ($poli) {
            ...
            if (is_numeric($poli)) {
                ...
                $w["LOWER(unit_nama)"] = strtolower($poli);
            }

            $data = $this->m_unit
                    ->group_start()
                        ->where($w)
                        ->or_where("LOWER(unit_kode)", strtolower($poli))
                    ->group_end()
                    ->where('unit_aktif', 'y')->get();

            if (!$data) {
                $this->form_validation->set_message(__FUNCTION__, 'Klinik Tidak Valid');
                return false;
            }
    ...
    public function validBayar($bayar){
        if ($bayar) {
            ...
            $data = $this->m_bayar
                    ->where('bayar_aktif', 'y')
                    ->group_start()
                        ->or_where("LOWER(bayar_nama)", strtolower($bayar))
                        ->or_where("LOWER(bayar_kode)", strtolower($bayar))
                    ->group_end()
                    ->get();

            if (!$data) {
                $this->form_validation->set_message(__FUNCTION__, 'Jenis Pembayaran Tidak Valid');
                return false;
            }
